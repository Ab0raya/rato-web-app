...
export const ProblemSection: React.FC = () => {
  ...
  return (
    <section className="relative py-28 sm:py-36 bg-[#0E0E11] border-t border-b border-[#2A2A2F]/60 overflow-hidden">
      {/* Background radial accent & subtle dot matrix grid */}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-20">
          ...
          {steps.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                className="p-8 rounded-3xl bg-[#161616]/90 backdrop-blur-xl border border-[#2A2A2F] text-left rtl:text-right space-y-4 relative group hover:border-[#FF6039]/50 hover:shadow-[0_0_30px_-5px_rgba(255,96,57,0.25)] transition-all duration-300 shadow-lg"
              >
                {/* Subtle corner kinetic dot */}
                <div className="absolute top-3 right-3 rtl:right-auto rtl:left-3 w-1.5 h-1.5 rounded-full bg-[#2A2A2F] group-hover:bg-[#FF6039] group-hover:shadow-[0_0_8px_#FF6039] transition-all" />

                <div className="flex items-center justify-between">
                  <div className="w-10 h-10 rounded-2xl bg-[#1A1A1E] border border-[#2A2A2F] group-hover:border-[#FF6039]/40 flex items-center justify-center text-[#FF6039] transition-colors">
                    <Icon className="w-5 h-5" />
                  </div>
                  <span className="text-xs font-mono font-bold text-[#9E9EA8]/60 group-hover:text-[#FF6039] transition-colors">
                    0{idx + 1}
                  </span>
                </div>
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#FF6039]/15 border border-[#FF6039]/30 text-xs font-bold text-[#FF6039] uppercase tracking-wider shadow-[0_0_20px_-5px_rgba(255,96,57,0.3)]">
